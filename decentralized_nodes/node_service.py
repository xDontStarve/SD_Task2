...
class NodeService:
    ...
    def put(self, key: str, value: str, delay: int) -> bool:
        ...
        for node in node_registrator.get_all_nodes():
            ...
            try:
                ...
                print(
                    "[NODE", id, "] [ERROR] One of the nodes took too long to respond during prepare phase, put operation failed.")
                return False
            transactionIdList[node_socket] = response.transactionId
            print("[Master] Node", node.node_id, ". Vote: ", response.voteCommit, "For transaction: ",
                  transactionIdList[node_socket])
        ...
        for node in node_registrator.get_all_nodes():
            ...
            try:
                ...
                print("[NODE", id, "] One of the nodes took too long to respond during commit phase, put operation failed.")
                return False
            if (commitResponse.success == False):
                # This case shouldn't happen, but if it does, some nodes may already have stored the data.
                return False
            print("[NODE", id, "] Node ", node.node_id, "Committed successfully the transaction: ",
                  transactionIdList[node_socket])
        return True
    ...
    def readVote(self, key: str, id: str) -> (int, str):
        pass

    def writeVote(self, key: str, id: str) -> int:
        pass
